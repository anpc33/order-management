                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                            <option value="pending" {{ old('status', $order->status) == 'pending' ? 'selected' : '' }}>{{ __('messages.order_status_pending') }}</option>
                            <option value="processing" {{ old('status', $order->status) == 'processing' ? 'selected' : '' }}>{{ __('messages.order_status_processing') }}</option>
                            <option value="shipping" {{ old('status', $order->status) == 'shipping' ? 'selected' : '' }}>{{ __('messages.order_status_shipping') }}</option>
                            <option value="completed" {{ old('status', $order->status) == 'completed' ? 'selected' : '' }}>{{ __('messages.order_status_completed') }}</option>
                            <option value="cancelled" {{ old('status', $order->status) == 'cancelled' ? 'selected' : '' }}>{{ __('messages.order_status_cancelled') }}</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="mb-3">
                    ...
                    <div id="products-container">
                        ...
                        <div class="row mb-2 product-row">
                            ...
                            <div class="col-md-4">
                                <input type="number" class="form-control quantity-input" name="products[{{ $index }}][quantity]" min="1" value="{{ $item->quantity }}" required>
                            </div>
                            <div class="col-md-2">
...
<script>
    $(document).ready(function() {
        ...
        $('#add-product').click(function() {
            ...
            newRow.find('select').attr('name', `products[${productCount}][id]`).val('');
